// controllers/authController.js - VERSIÓN SECURA Y CORREGIDA
 ...
 * CORREGIDA para ignorar campos no esperados.
 */
const validateAndSanitizeAdditionalData = (rawData) => {
    ...
    // Define los campos de perfil que se esperan y deben ser validados
    const profileFields = [
        'first_name',
        'last_name',
        'phone',
    ...
    try {
        // Itera sobre los campos esperados para evitar procesar datos no deseados.
        for (const field of profileFields) {
            if (rawData[field] !== undefined && rawData[field] !== null) {
                if (field === 'first_name' || field === 'last_name') {
                    validatedData[field] = sanitizeAndValidate.validateName(rawData[field], field);
                } else if (field === 'phone') {
                    validatedData[field] = sanitizeAndValidate.validatePhone(rawData[field]);
                } else if (field === 'city') {
                    validatedData[field] = sanitizeAndValidate.validateCity(rawData[field]);
                } else if (field === 'profile_picture_url') {
                    validatedData[field] = sanitizeAndValidate.validateProfilePictureUrl(rawData[field]);
                }
            }
        }
        return validatedData;
    } catch (error) {
        // El error se captura si la validación de un campo específico falla.
...
const handleFirebaseLogin = async (req, res) => {
    ...
    try {
        // 1. Obtener el token y los datos adicionales del CUERPO de la solicitud
        const { idToken, first_name, last_name, phone } = req.body;

        if (!idToken) {
            // El error 400 que estabas recibiendo se debe a esto.
        ...
        // 6. Validar datos adicionales del cuerpo
